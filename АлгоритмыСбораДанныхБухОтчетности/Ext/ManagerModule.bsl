...
Функция СформироватьРасшифровкуАлгоритма(ТипПоказателя, Счета, КорСчета, НастройкиОтбора = Неопределено) Экспорт
	...
	ИначеЕсли ТипПоказателя = "ОБ" Тогда
		...
		Если КорСчета.Количество() = 1 И КорСчета[0].Вид = ВидСчета.Пассивный Тогда
...
// Возвращаемое значение:
//  Массив из Строка
Функция ТипыПоказателя() Экспорт

	ТипыПоказателя = Новый Массив;
